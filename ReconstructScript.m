...
dirName = 'C:\Users\Xiaonan Hui\Documents\zijing zhang\data 02072020\214\';
% fileName = 'wo_1224';
opts.calibType = 1;

% Load the raw data.
load([dirName, 'data_wo03','.mat']); % Looking at 2 minute cases
...
% Load the raw data.
load([dirName, 'data_2ballon03','.mat']); 
...
%% post process of Reconstruction image
...
T=adaptthresh(ReconsDistNorm,'neigh',[9,9,9]);
Tmax=max(T(:))
ReconsDistNorm(ReconsDistNorm<Tmax)=0;


ReconsDistNorm = imgaussfilt3(ReconsDistNorm);
ReconsDistNorm0=ReconsDistNorm;

imgBrightnessf = csaps({x_v,y_v,z_v},ReconsDistNorm,0.9995);
imgSpline = fnval(imgBrightnessf,{x_v,y_v,z_v}) ;
imgSpline0= imgSpline;

Ts=adaptthresh(imgSpline,'neigh',[15,15,15]);
Tsmax=max(Ts(:))
%Tsmax=0.15;
imgSplineMax=max(imgSpline(:));
imgSpline(imgSpline<(Tsmax*imgSplineMax))=0;

[gx1,gy1,gz1] = gradient(imgSpline);
 g1=sqrt(gx1.^2 +gy1.^2 +gz1.^2);
 
 imgSplineBI=imgSpline;
 imgSplineBI(imgSplineBI>0)=1;
 H = fspecial3('sobel');
 edgesHor = imfilter(imgSplineBI,H,'replicate');
 edgesHor=abs(edgesHor);
 
BW = edge3(imgSplineBI,'Sobel',0.2);
BW=abs(BW);
%% Plotting.
figure
...
figure
h = slice(X_V, Y_V, Z_V,imgSpline ,x_v,y_v,z_v);
% imgComplexAbs(imgComplexAbs == 0) = NaN;
% h = slice(X_V, Y_V, Z_V, imgComplexAbs,x_v,y_v,z_v);
xlabel('x / m','FontSize',14);
ylabel('y / m','FontSize',14);
...
figure
h = slice(X_V, Y_V, Z_V,BW ,x_v,y_v,z_v);
% imgComplexAbs(imgComplexAbs == 0) = NaN;
% h = slice(X_V, Y_V, Z_V, imgComplexAbs,x_v,y_v,z_v);
xlabel('x / m','FontSize',14);
ylabel('y / m','FontSize',14);
...
[cDistribution, clusters,centroidDist] = i4block_components(imgSpline, roomSize, voxelSize);
...
fprintf('\n');
opts.distTh = 0.1; % distance threshold, clusters with centers closer than this will be combined
opts.XYdistTh = 0.1;
opts.elemNumTh = 0.61; % clusters with element number less than 60% of the maximum will be rejected
opts.minHeightRatio = 0.6; % Minimum height ratio compared to largest object, exact ht depends on voxel size etc.
clusterOut = clusterProcess(clusters,opts);
...
znum=round(centroid(3)/pixel)+1;

xyplane_lse=imgSpline(:,:,znum);
xyplane_lse=xyplane_lse-Tsmax;
[x_fit_lse,y_fit_lse,area,p]=level_set(x_v,y_v,xyplane_lse,pixel);


figure()
plot(x_fit_lse,y_fit_lse)
xlim([0 1.5]);
ylim([0 1.5]);
%perim=regionprops(xyplane,'Perimeter');
%perim=pixel*perim.Perimeter*100;


%xyplane=BW(:,:,znum);
% [row,col] = find(xyplane>0);   If use imgspline after threshold 
% x_row=x_v(col);  %guoyi set x =col 
% y_row=y_v(row);
% [theta,rho] = cart2pol(x_row-mean(x_row),y_row-mean(y_row));
% fitobject = fit(theta',rho','poly1');
% figure()
% plot(fitobject,theta',rho')
% theta_fit=-pi+0.01:0.01:pi;
...
% figure
% slice(X_V, Y_V, Z_V,BW ,[],[],z_v(znum));
% xyplane=xyplane*256;
% [xplane,yplane]=meshgrid(x_v,y_v);
% x_sample=0:0.001:1.5;
% y_sample=0:0.001:1.5;
% [x_sampleplane,y_sampleplane]=meshgrid(x_sample,y_sample);
% vq = griddata(xplane,yplane,xyplane,x_sampleplane,y_sampleplane,'cubic');
% B = imgaussfilt(vq);
% 
% figure
% image(x_sample,y_sample,B)
%  figure
% image(x_v,y_v,xyplane)
...
% zlabel('z coordinate (cm)');
tComp = toc;
fprintf('Time for processing = %4.3f s.\n',tComp);
